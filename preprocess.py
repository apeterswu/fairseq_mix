...
def get_parser():
    ...
    parser.add_argument('--srcdict-sen-piece', metavar='FP', help='reuse src sentence piece dictionary')
    parser.add_argument('--tgtdict-sen-piece', metavar='FP', help='reuse tgt sentence piece dictionary')
...
def main(args):
    ...
    if args.joined_dictionary:
        ...
        tgt_dict = src_dict
        src_dict_sen_piece = build_dictionary(set([
            train_path(lang+'.sen_piece')
            for lang in [args.source_lang, args.target_lang]
        ]))
        tgt_dict_sen_piece = src_dict_sen_piece
    else:
        if args.srcdict:
        ...
        if args.srcdict_sen_piece:
            src_dict_sen_piece = dictionary.Dictionary.load(args.srcdict_sen_piece)
        else:
            assert args.trainpref, "--trainpref must be set if --srcdict_sen_piece is not specified"
            src_dict_sen_piece = build_dictionary(train_path(args.source_lang + '.sen_piece'))
        if target:
            if args.tgtdict:
            ...
            if args.tgtdict_sen_piece:
                tgt_dict_sen_piece = dictionary.Dictionary.load(args.tgtdict_sen_piece)
            else:
                assert args.trainpref, "--trainpref must be set if --tgtdict_sen_piece is not specified"
                tgt_dict_sen_piece = build_dictionary([train_path(args.target_lang + '.sen_piece')])
    ...
    src_dict_sen_piece.finalize(
        threshold=args.thresholdsrc,
        nwords=args.nwordssrc,
        padding_factor=args.padding_factor,
    )
    src_dict_sen_piece.save(dict_path(args.source_lang+'.sen_piece'))
    if target:
        if not args.joined_dictionary:
            ...
            tgt_dict_sen_piece.finalize(
                threshold=args.thresholdtgt,
                nwords=args.nwordstgt,
                padding_factor=args.padding_factor,
            )
        tgt_dict.save(dict_path(args.target_lang))
        tgt_dict_sen_piece.save(dict_path(args.target_lang+'.sen_piece'))
    ...
    make_all(args.source_lang+'.sen_piece')
    if target:
        make_all(args.target_lang+'.sen_piece')

    print('| Wrote preprocessed data to {}'.format(args.destdir))

    if args.alignfile:   # not used
