...
python $CODE_PATH/generate.py $DATA_PATH --source-lang $SRC --target-lang $TGT --path ${MODEL_PATH}/checkpoint_best.pt --senpiece-model $SENPIECE_MODEL_PATH --batch-size 128 --beam $BEAM --lenpen $LENP --quiet --remove-bpe | tee $PREFIX/translation/$MODEL.bleu
