...
def _upgrade_state_dict(state):
        ...
        state['optimizer_history'] = [
            {
                ...
                'best_loss_sen_piece': state['best_loss_sen_piece'],
                'best_overall_loss': state['best_overall_loss'],
            },
        ]
        state['last_optimizer_state'] = state['optimizer']
        del state['optimizer']
        del state['best_loss']
        del state['best_loss_sen_piece']
        del state['best_overall_loss']
    ...
    if 'epoch' in state and 'extra_state' not in state:
        state['extra_state'] = {
            ...
            'val_loss_sen_piece': state['val_loss_sen_piece'],
            'val_overall_loss': state['val_overall_loss'],
        }
        del state['epoch']
        del state['batch_offset']
        del state['val_loss']
        del state['val_loss_sen_piece']
        del state['val_overall_loss']
    ...
    if 'lr_scheduler_state' not in state['optimizer_history'][-1]:
        state['optimizer_history'][-1]['lr_scheduler_state'] = {
            ...
            'best_sen_piece': state['optimizer_history'][-1]['best_loss_sen_piece'],
            'best_overall': state['optimizer_history'][-1]['best_overall_loss'],
        }
        del state['optimizer_history'][-1]['best_loss']
        del state['optimizer_history'][-1]['best_loss_sen_piece']
        del state['optimizer_history'][-1]['best_overall_loss']
...
def post_process_prediction(hypo_tokens, src_str, alignment, align_dict, tgt_dict, remove_bpe, to_list=False):
    from fairseq import tokenizer
    if to_list:
        hypo_str = tgt_dict.to_list(hypo_tokens)
    else:
