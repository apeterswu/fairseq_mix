...
class SequenceScorerRanking(object):
    ...
    def score_batched_itr(self, data_itr, cuda=False, timer=None):
        ...
        for sample in data_itr:
            ...
            if timer is not None:
                timer.start()
            pos_scores, attn, same_pos = self.score_ranking(s)
    ...
    def score_ranking(self, sample):
        ...
        max_pos = torch.argmax(avg_probs, dim=-1)  # max positions along the vocab
        same_pos = (max_pos == sample['target'].data)
        avg_probs = avg_probs.gather(
            dim=2,
            index=sample['target'].data.unsqueeze(-1),
        )
        return avg_probs.squeeze(2), avg_attn, same_pos
