...
@register_model('transformer_stack')
class TransformerModelStack(FairseqModel):
    """
    ...
    def add_args(parser):
        ...
        parser.add_argument('--share-parameter', action='store_true',
                            help='whether to share parameter for two embeddings')
    ...
    def build_model(cls, args, task):
        ...
        # src_dict_sen_piece = None
        # tgt_dict_sen_piece = None
        ...
        return TransformerModelStack(encoder, decoder)
...
class TransformerEncoder(FairseqEncoder):
    ...
    def forward(self, src_tokens, src_lengths,
        ...
        if self.embed_positions_sen_piece is not None:
            x_sen_piece = x_sen_piece + self.embed_positions_sen_piece(src_sen_piece_tokens)
        ...
        encoder_padding_mask_sen_piece = src_sen_piece_tokens.eq(self.padding_idx_sen_piece)
        ...
        if self.normalize:
            x = self.layer_norm(x)
            x_sen_piece = self.layer_norm(x_sen_piece) if self.share_parameter \
    ...
    def forward(self, encoder_out=None, prev_output_tokens=None,
        ...
        if prev_output_sen_piece_tokens is not None:
            ...
            if self.adaptive_softmax is None:
                # project back to size of vocabulary
                if self.share_input_output_embed:
                    x_sen_piece = F.linear(x_sen_piece, self.embed_tokens_sen_piece.weight)
                else:
                    x_sen_piece = F.linear(x_sen_piece, self.embed_out_sen_piece)
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, args):
        ...
        if not self.share_parameter:
            ...
            self.cross_attn_sen_piece = MultiheadAttention(
                self.embed_dim, args.encoder_attention_heads,
                dropout=args.attention_dropout,
            )
            ...
            self.layer_norms_sen_piece = nn.ModuleList([LayerNorm(self.embed_dim) for i in range(3)])
        self.dropout = args.dropout
    ...
    def forward(self, x, encoder_padding_mask,
        ...
        if not self.share_parameter:
            ...
            x_sen_piece = self.maybe_layer_norm(0, x_sen_piece, sen_piece=True, before=True)
            # self-attention
            ...
            x_sen_piece, _ = self.self_attn_sen_piece(query=x_sen_piece, key=x_sen_piece, value=x_sen_piece,
                                                      key_padding_mask=encoder_padding_mask_sen_piece)
            x_sen_piece = F.dropout(x_sen_piece, p=self.dropout, training=self.training)
            x_sen_piece = residual_sen_piece + x_sen_piece
            x_sen_piece = self.maybe_layer_norm(0, x_sen_piece, sen_piece=True, after=True)

            residual = x
            residual_sen_piece = x_sen_piece
            x = self.maybe_layer_norm(1, x, before=True)
            x_sen_piece = self.maybe_layer_norm(1, x_sen_piece, sen_piece=True, before=True)
            # cross-attention
            # TODO: remove the cross-attn, try again...
            x, _ = self.cross_attn(query=x, key=residual_sen_piece, value=residual_sen_piece,
                                   key_padding_mask=encoder_padding_mask_sen_piece)
            x_sen_piece, _ = self.cross_attn_sen_piece(query=x_sen_piece, key=residual, value=residual,
            ...
            x_sen_piece = F.dropout(x_sen_piece, p=self.dropout, training=self.training)
            x_sen_piece = residual_sen_piece + x_sen_piece
            x_sen_piece = self.maybe_layer_norm(1, x_sen_piece, sen_piece=True, after=True)

        else:
            x = self.maybe_layer_norm(0, x, before=True)
            x_sen_piece = self.maybe_layer_norm(0, x_sen_piece, before=True)
            # self_attention
            x, _ = self.self_attn(query=x, key=x, value=x, key_padding_mask=encoder_padding_mask)
            x_sen_piece, _ = self.self_attn(query=x_sen_piece, key=x_sen_piece, value=x_sen_piece,
                                            key_padding_mask=encoder_padding_mask_sen_piece)
            x = F.dropout(x, p=self.dropout, training=self.training)
            x = residual + x
            x = self.maybe_layer_norm(0, x, after=True)
            ...
            x_sen_piece = F.dropout(x_sen_piece, p=self.dropout, training=self.training)
            x_sen_piece = residual_sen_piece + x_sen_piece
            x_sen_piece = self.maybe_layer_norm(0, x_sen_piece, after=True)

            residual = x
            residual_sen_piece = x_sen_piece
            x = self.maybe_layer_norm(1, x, before=True)
            x_sen_piece = self.maybe_layer_norm(1, x_sen_piece, before=True)
            # cross_attention
            x, _ = self.cross_attn(query=x, key=x_sen_piece, value=x_sen_piece,
                                   key_padding_mask=encoder_padding_mask_sen_piece)
            x_sen_piece, _ = self.cross_attn(query=x_sen_piece, key=residual, value=residual,
                                             key_padding_mask=encoder_padding_mask)
            x = F.dropout(x, p=self.dropout, training=self.training)
            x = residual + x
            x = self.maybe_layer_norm(1, x, after=True)
            ...
            x_sen_piece = F.dropout(x_sen_piece, p=self.dropout, training=self.training)
            x_sen_piece = residual_sen_piece + x_sen_piece
            x_sen_piece = self.maybe_layer_norm(1, x_sen_piece, after=True)
        ...
        residual_sen_piece = x_sen_piece
        x_sen_piece = self.maybe_layer_norm(2, x_sen_piece, sen_piece=False if self.share_parameter else True, before=True)
        x_sen_piece = F.relu(self.fc1(x_sen_piece) if self.share_parameter else self.fc1_sen_piece(x_sen_piece))
        x_sen_piece = F.dropout(x_sen_piece, p=self.relu_dropout, training=self.training)
        x_sen_piece = self.fc2(x_sen_piece) if self.share_parameter else self.fc2_sen_piece(x_sen_piece)
        x_sen_piece = F.dropout(x_sen_piece, p=self.dropout, training=self.training)
        x_sen_piece = residual_sen_piece + x_sen_piece
        x_sen_piece = self.maybe_layer_norm(2, x_sen_piece, sen_piece=False if self.share_parameter else True, after=True)
        return x, x_sen_piece

    def maybe_layer_norm(self, i, x, sen_piece=False, before=False, after=False):
        assert before ^ after
        if after ^ self.normalize_before:
            if sen_piece:
                return self.layer_norms_sen_piece[i](x)
            else:
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def forward(self, x, x_sen_piece, encoder_out, encoder_padding_mask,
        ...
        if x_sen_piece is not None:
            residual_sen_piece = x_sen_piece
            x_sen_piece = self.maybe_layer_norm(self.self_attn_layer_norm if self.share_parameter
        ...
        if x is not None:
            if self.encoder_attn is not None:
                ...
                # Lijun TODO: residual replace or not?
                inner_residual = x
                x = self.maybe_layer_norm(self.encoder_attn_layer_norm_sen_piece, x, before=True)
                # dec1 attn on enc2
                x, _ = self.encoder_attn_sen_piece(
                    query=x,
                    key=encoder_out_sen_piece,
                    value=encoder_out_sen_piece,
                    key_padding_mask=encoder_padding_mask_sen_piece,
                    incremental_state=incremental_state,  # not to reuse the key of encoder_attn
                    sen_piece=True,
                    static_kv=True,
                    need_weights=(not self.training and self.need_attn),
                )
                x = F.dropout(x, p=self.dropout, training=self.training)
                x = inner_residual + residual + x
                x = self.maybe_layer_norm(self.encoder_attn_layer_norm_sen_piece, x, after=True)
        ...
        if x_sen_piece is not None:
            if self.encoder_attn is not None:
                residual_sen_piece = x_sen_piece
                x_sen_piece = self.maybe_layer_norm(self.encoder_attn_layer_norm_sen_piece, x_sen_piece, before=True)
                # dec2 attn to enc2
                ...
                x_sen_piece = F.dropout(x_sen_piece, p=self.dropout, training=self.training)
                x_sen_piece = residual_sen_piece + x_sen_piece
                x_sen_piece = self.maybe_layer_norm(self.encoder_attn_layer_norm_sen_piece, x_sen_piece, after=True)

                # Lijun TODO: residual replace or not?
                inner_residual_sen_piece = x_sen_piece
                x_sen_piece = self.maybe_layer_norm(self.encoder_attn_layer_norm, x_sen_piece, before=True)
                # dec2 attn to enc1
                x_sen_piece, _ = self.encoder_attn(
                    query=x_sen_piece,
                    key=encoder_out,
                    value=encoder_out,
                    key_padding_mask=encoder_padding_mask,
                ...
                x_sen_piece = F.dropout(x_sen_piece, p=self.dropout, training=self.training)
                x_sen_piece = inner_residual_sen_piece + residual_sen_piece + x_sen_piece
                x_sen_piece = self.maybe_layer_norm(self.encoder_attn_layer_norm, x_sen_piece, after=True)

            residual_sen_piece = x_sen_piece
            x_sen_piece = self.maybe_layer_norm(self.final_layer_norm if self.share_parameter
...
@register_model_architecture('transformer_stack', 'transformer_stack')
def base_architecture(args):
    ...
    args.share_parameter = getattr(args, 'share_parameter', False)


@register_model_architecture('transformer_stack', 'transformer_stack_iwslt_de_en')
...
@register_model_architecture('transformer_stack', 'transformer_stack_wmt_en_de')
def transformer_wmt_en_de(args):
    base_architecture(args)


# parameters used in the "Attention Is All You Need" paper (Vaswani, et al, 2017)
@register_model_architecture('transformer_stack', 'transformer_stack_vaswani_wmt_en_de_big')
...
@register_model_architecture('transformer_stack', 'transformer_stack_vaswani_wmt_en_fr_big')
def transformer_vaswani_wmt_en_fr_big(args):
    args.dropout = getattr(args, 'dropout', 0.1)
    transformer_vaswani_wmt_en_de_big(args)


@register_model_architecture('transformer_stack', 'transformer_stack_wmt_en_de_big')
...
@register_model_architecture('transformer_stack', 'transformer_stack_wmt_en_de_big_t2t')
