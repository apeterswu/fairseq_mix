...
@register_model('transformer_dec_one')
class TransformerDecOneModel(FairseqModel):
    """
    ...
    def add_args(parser):
        ...
        parser.add_argument('--drop-first', action='store_true', default=False,
                            help='when average two representations, first drop, then add; '
                                 'otherwise, first add, then drop.')
    ...
    def build_model(cls, args, task):
        ...
        return TransformerDecOneModel(encoder, decoder)
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, args):
        ...
        if not self.ffn_share:
            ...
            # self.layer_norms_sen_piece = nn.ModuleList([LayerNorm(self.embed_dim) for i in range(2)])
        self.dropout = args.dropout
    ...
    def forward(self, x, encoder_padding_mask, x_sen_piece, encoder_padding_mask_sen_piece):
        ...
        if not self.all_share:
            ...
            x1_sen_piece, _ = self.self_attn_sen_piece(query=x, key=x_sen_piece, value=x_sen_piece,
                                                       key_padding_mask=encoder_padding_mask_sen_piece)
            if self.drop_first:
                x1 = F.dropout(x1, p=self.dropout, training=self.training)
                x1_sen_piece = F.dropout(x1_sen_piece, p=self.dropout, training=self.training)

            x2, _ = self.self_attn(query=x_sen_piece, key=x, value=x, key_padding_mask=encoder_padding_mask)
            x2_sen_piece, _ = self.self_attn_sen_piece(query=x_sen_piece, key=x_sen_piece, value=x_sen_piece,
                                                       key_padding_mask=encoder_padding_mask_sen_piece)
            if self.drop_first:
                x2 = F.dropout(x2, p=self.dropout, training=self.training)
                x2_sen_piece = F.dropout(x2_sen_piece, p=self.dropout, training=self.training)

            # add after dropout, then residual
            x = 0.5 * x1 + 0.5 * x1_sen_piece * self.get_drop_path_ratio()
            x_sen_piece = 0.5 * x2 * self.get_drop_path_ratio() + 0.5 * x2_sen_piece
            ...
            x1, _ = self.self_attn(query=x, key=x, value=x, key_padding_mask=encoder_padding_mask)
            x1_sen_piece, _ = self.self_attn(query=x, key=x_sen_piece, value=x_sen_piece,
                                             key_padding_mask=encoder_padding_mask_sen_piece)
            if self.drop_first:
                x1 = F.dropout(x1, p=self.dropout, training=self.training)
                x1_sen_piece = F.dropout(x1_sen_piece, p=self.dropout, training=self.training)
            ...
            x2, _ = self.self_attn(query=x_sen_piece, key=x, value=x, key_padding_mask=encoder_padding_mask)
            x2_sen_piece, _ = self.self_attn(query=x_sen_piece, key=x_sen_piece, value=x_sen_piece,
                                             key_padding_mask=encoder_padding_mask_sen_piece)
            if self.drop_first:
                x2 = F.dropout(x2, p=self.dropout, training=self.training)
                x2_sen_piece = F.dropout(x2_sen_piece, p=self.dropout, training=self.training)
        ...
        # x_sen_piece = self.maybe_layer_norm(1, x_sen_piece, sen_piece=False if self.ffn_share else True, before=True)
        ...
        # x_sen_piece = self.maybe_layer_norm(1, x_sen_piece, sen_piece=False if self.ffn_share else True, after=True)
    ...
    def forward(self, x, x_sen_piece, encoder_out, encoder_padding_mask,
        ...
        if x is not None:
            if self.encoder_attn is not None:
                ...
                # decoder, dec1 attn to enc2
                x_enc_sen_piece, _ = self.encoder_attn(
                    query=x,
                ...
                if self.drop_first:
                    ...
                    x_enc_sen_piece = F.dropout(x_enc_sen_piece, p=self.dropout, training=self.training)

                x = 0.5 * x_enc + 0.5 * x_enc_sen_piece * self.get_drop_path_ratio()
                if not self.drop_first:
                    x = F.dropout(x, p=self.dropout, training=self.training)
                x = residual + x
        ...
        if x_sen_piece is not None:
            ...
            if not self.all_share:
                # decoder, dec2 attn to enc1
                x_sen_piece_enc, _ = self.encoder_attn_sen_piece(
                    query=x_sen_piece,
                    key=encoder_out,
                    value=encoder_out,
                    key_padding_mask=encoder_padding_mask,
                ...
                # decoder, dec2 attn to enc2
                x_sen_piece_enc_sen_piece, attn_sen_piece = self.encoder_attn_sen_piece(
                    query=x_sen_piece,
                    key=encoder_out_sen_piece,
                    value=encoder_out_sen_piece,
                    key_padding_mask=encoder_padding_mask_sen_piece,
                ...
                # decoder, dec2 attn to enc1
                x_sen_piece_enc, _ = self.encoder_attn(
                    query=x_sen_piece,
                    key=encoder_out,
                    value=encoder_out,
                    key_padding_mask=encoder_padding_mask,
                ...
                # decoder, dec2 attn to enc2
                x_sen_piece_enc_sen_piece, attn_sen_piece = self.encoder_attn(
                    query=x_sen_piece,
                    key=encoder_out_sen_piece,
                    value=encoder_out_sen_piece,
                    key_padding_mask=encoder_padding_mask_sen_piece,
            ...
            if self.drop_first:
                x_sen_piece_enc = F.dropout(x_sen_piece_enc, p=self.dropout, training=self.training)
                x_sen_piece_enc_sen_piece = F.dropout(x_sen_piece_enc_sen_piece, p=self.dropout, training=self.training)
            x_sen_piece = 0.5 * x_sen_piece_enc * self.get_drop_path_ratio() + 0.5 * x_sen_piece_enc_sen_piece
            if not self.drop_first:
                x_sen_piece = F.dropout(x_sen_piece, p=self.dropout, training=self.training)
            x_sen_piece = residual_sen_piece + x_sen_piece
            x_sen_piece = self.maybe_layer_norm(self.encoder_attn_layer_norm if self.all_share
...
@register_model_architecture('transformer_dec_one', 'transformer_dec_one')
def base_architecture(args):
    ...
    args.drop_first = getattr(args, 'drop_first', False)


@register_model_architecture('transformer_dec_one', 'transformer_dec_one_iwslt_de_en')
...
@register_model_architecture('transformer_dec_one', 'transformer_dec_one_wmt_en_de')
def transformer_wmt_en_de(args):
    base_architecture(args)


# parameters used in the "Attention Is All You Need" paper (Vaswani, et al, 2017)
@register_model_architecture('transformer_dec_one', 'transformer_dec_one_vaswani_wmt_en_de_big')
...
@register_model_architecture('transformer_dec_one', 'transformer_dec_one_vaswani_wmt_en_fr_big')
def transformer_vaswani_wmt_en_fr_big(args):
    args.dropout = getattr(args, 'dropout', 0.1)
    transformer_vaswani_wmt_en_de_big(args)


@register_model_architecture('transformer_dec_one', 'transformer_dec_one_wmt_en_de_big')
...
@register_model_architecture('transformer_dec_one', 'transformer_dec_one_wmt_en_de_big_t2t')
